def build_backstory(industry_focus: str, ignore_rule: str) -> str:
    ...
    return f"""You are a deterministic, zero-creativity machine. Your ONLY function is to act as a highlighter pen for {industry_focus}.
        ...
        - NEVER write any explanations, thoughts, intro/outro summaries, or metadata wrappers (such as "For the article..., I extracted..."). Any text generated by you that is not strictly present in the original source text is an absolute failure."""
...
def build_task_template(
    ...
    return f"""
        ...
        5. Output ONLY the raw extracted text blocks natively, excluding all conversational fillers, prefixes, numbering, or labels.
           - **CRITICAL**: Do NOT add any conversational prefixes, thoughts, explanation wrappers, or introductory sentences (such as "For the article..., I extracted..." or "Here is the extraction..."). Start directly with the exact extracted sentences from the source text. 
           - **ZERO TOLERANCE**: Every single sentence in your output MUST be a strict 100% exact substring match from the original source text. Any sentence or phrase created by you that is NOT in the original article will violate the core constraint.
